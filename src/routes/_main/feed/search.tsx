...
function SearchPage() {
  ...
  const { data: results = [], isLoading } = usePostsList(
    q ? { q } : undefined,
  );
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6">
      ...
      <PageHeader eyebrow="Forum" title="Search Results" className="mb-6" />
        ...
        <div className="flex gap-2 max-w-xl">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="h-10 w-full rounded border border-border bg-card pl-9 pr-3 text-sm text-foreground outline-none ring-primary focus:ring-1 placeholder:text-muted-foreground"
            />
          </div>
          <button
            type="submit"
            className="rounded bg-primary px-4 py-2 label-military text-primary-foreground hover:bg-primary/85 transition-colors"
          >
            Search
          </button>
        </div>
        ...
        <>
          ...
          <div className="flex flex-col gap-1.5">
            {results.map((post) => (
              <article
                key={post.id}
                className="flex gap-3 border border-border bg-card p-4 transition-colors hover:border-primary/30 hover:bg-accent"
              >
